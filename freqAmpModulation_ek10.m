...
frameVec = [12,13,14,15]; 
parentDir = '/media/ahsan/ds00/Ekman_Ugeo10_2048x2048x64_Lz1500';
...
ul_binwidth = 0.0248;
midval_ul = -1.2152:ul_binwidth:1.3603;
...
save('freqAmpMod_ek10.mat','us_conditional_u_l','midval_ul','pdf_N_u_l',...
...
% xlabel('x/\delta');
% SPR = sprintf('%s%s','','ek10_u_l_u_s.eps');  
% print(formatEng,SPR);
...
set(gcf,'Renderer','painters');
hold all;
ul_val_vec = [35 40 45 50 55 60 65];% 50 is u_l = 0;
for i = 1:length(ul_val_vec)
    plot(heightVec(2:end), 3.*uw_std_cond_u_l(ul_val_vec(i),2:end)./u_star^2, LineStyles2{i},'MarkerSize',4); 
end
lgndVal = cell([numel(ul_val_vec) 1]);
for ii = 1:numel(ul_val_vec)
    lgndVal{ii} = sprintf('u_L = %2.3f',midval_ul(ul_val_vec(ii)));
...
set(gca, 'Xscale','log');
for i = 1:length(ul_val_vec)
    plot(heightVec(2:end), uw_mean_cond_u_l(ul_val_vec(i),2:end)./u_star^2,LineStyles1{i},'MarkerSize',4);
    plot(heightVec(2:end),-3.*uw_std_cond_u_l(ul_val_vec(i),2:end)./u_star^2, LineStyles2{i},'MarkerSize',4);
end
xlabel('$z/\delta$','Interpreter','Latex');
ylabel('$\left < {u^{\prime} w^{\prime}}\right >/u_*^2$','Interpreter','Latex');
xlim([0.007 1.1]); ylim([-2.0 4.5]);
set(gca, ...
  'Box'         , 'off'     , ...
  'TickDir'     , 'out'     , ...
  'TickLength'  , [.02 .02] , ...
  ...
  'LineWidth'   , 0.5         );
SPR = sprintf('%s','ek10_u_l_uw_std.eps');  
print(formatEng,SPR);

% plot Delta u_s^2 vs y
% ul_val_vec = [37 42 47 52 57 62 67];% 53 is u_l = 0;
tmp_vec = [44 47 53 56];
figure(); 
% set(gcf,'Renderer','OpenGL');
set(gcf, 'PaperUnits', 'inches', 'PaperPosition', [0 0 figWidth figHeight]);
...
set(gca, ...
  ...
  'XTick'       , [.001 0.01 0.1 1], ...
  'LineWidth'   , 0.5         );
SPR = sprintf('%s','ek10_u_s2_z_multiple_u_l_0.eps');  
print(formatEng,SPR);
...
%   'MarkerSize'  , 4, ...
%   'LineWidth'   , 0.5         );
% SPR = sprintf('%s','ek10_u_s2_uL_multiple_z_ul_0.eps');  
% print(formatEng,SPR);
...
ul_val_vec = [47 50 53];% 53 is u_l = 0;
figure(); 
set(gcf, 'PaperUnits', 'inches', 'PaperPosition', [0 0 figWidth figHeight]);
set(gcf,'Renderer','painters');
hold all;
...
set(gca, ...
  ...
  'XTick'       , [.001 0.01 0.1 1], ...
  'LineWidth'   , 0.5         );
SPR = sprintf('%s','ek10_kx_ul_z.eps');  
print(formatEng,SPR);
...
% SPR = sprintf('%s','ek10_pcolor_pdf_uL_against_uL_z.eps');  
% print(formatEng,SPR);
% 
...
% hcbar = colorbar('northoutside');
% ylabel(hcbar, 'u_s^{2+}');
% SPR = sprintf('%s','ek10_pcolor_us2_against_uL_z.eps');  
% print(formatEng,SPR);
