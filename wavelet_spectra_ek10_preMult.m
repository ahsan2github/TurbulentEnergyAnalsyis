...
hf = figure(); set(gcf,'Renderer','painters');
set(hf,'Visible','off');
...
xlim([0.0003 100]);
xx = [2 40];
loglog(xx,xx.^(-2/3).*0.75,'LineWidth',2); % -2/3 slope
text(7, 0.25, '-2/3','FontSize',8);
set(gca,'XScale','log'); set(gca,'YScale','log');
box on;
SPR = sprintf('%s%s','','wavelet_n_fourier_spectra_ek10_pre.eps');  
set(gcf, 'Color', 'w');
print(formatEng,SPR);
