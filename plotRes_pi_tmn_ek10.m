parentDir = '/raid/home/mohammad/Ekman_Ugeo10_2048x2048x64_Lz1500';
...
data = load('wav_pi_tmn_ek10_fixed.mat');
zvec = (0:Nz-1).*dz;
epsilon = ustar^3./dz/2/0.4;
% epsilon_vol_avg = trapz(zvec,epsilon)/zvec(end);
% epsilon(:) = epsilon_vol_avg;
pi_energy = data.frame_energy_pi;
pi_energy2 = data.frame_energy_pi2;
LineStyles = {'-', '--','-.',':','-+','-o','-s','-d','-^','-*'}; % 10 types total
figWidth = 3.5; figHeight = figWidth/1.1667;  formatEng = '-depsc2';
...
nframe = 6; %length(pi_energy);
pi_n_data_frame = zeros([numel(n_pos_st:n_pos_end) numel(refLevels) nframe]);
pi_n_data_sigma_frame = zeros([numel(n_pos_st:n_pos_end) numel(refLevels) nframe]);
pi_n_data_frame2 = zeros([numel(n_pos_st:n_pos_end) numel(refLevels) nframe]);
pi_n_data_sigma_frame2 = zeros([numel(n_pos_st:n_pos_end) numel(refLevels) nframe]);
for ff = 1:nframe
...
'LineWidth',    0.5);
set(gcf, 'Color', 'w');
SPR = sprintf('%s%s','','pi_ek10_diff_n_by_u3_dz.eps');  
set(gcf, 'Color', 'w');
print(formatEng,SPR);
...
'LineWidth',    0.5);
set(gcf, 'Color', 'w');
SPR = sprintf('%s%s','','pi_ek10_diff_n_by_u3_dz_2.eps');  
print(formatEng,SPR);
...
SPR = sprintf('%s%s','','pi_ek10_fixed_h_by_u3_dz.eps');  
set(gcf, 'Color', 'w');
print(formatEng,SPR);
