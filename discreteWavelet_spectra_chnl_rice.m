...
%     plot(kwave_delta, wavelet_spectra_u_frame(kk,:)+ wavelet_sigma_u_frame(kk,:),'+'); 
    ...
    xlabel('$k_x \delta$','Interpreter','Latex','FontSize',10);
    ylabel('$E_{11}(k_x)u_*^{-2}\delta^{-1}$','Interpreter','Latex','FontSize',10);
end
xlim([0.05 100]);
ylim([2e-9 0.0003]);
xx = [10 80];
loglog(xx,xx.^(-5/3).*0.0001,'LineWidth',2); % -5/3 slope
text(25, 0.000001, '-5/3','FontSize',8);
xx = [0.4 10];
loglog(xx,xx.^(-1).*0.000035,'LineWidth',2); % -1 slope
text(2.5, 0.000033, '-1','FontSize',8);
text(0.1,0.1,'(a)','Units','Normalized','FontSize',8);
set(gca,'XScale','log'); set(gca,'YScale','log');
SPR = sprintf('%s%s','','wavelet_n_fourier_spectra_chnl.eps');  
set(gcf, 'Color', 'w');
print(formatEng,SPR);
