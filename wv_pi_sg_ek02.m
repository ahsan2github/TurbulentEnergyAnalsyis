...
BLH = 550.69;
refLevels = 3:2:47; % total 21 levels
...
for ff = 1:length(frameVec)
    ...
    sfl = '/raid/home/mohammad/stressFiles';
    fn = [sfl '/ek02_tau12_'  frameStr   '.bin'];
    fh = fopen(fn, 'r');
    ...
    fn = [sfl '/ek02_tau23_'  frameStr   '.bin'];
    fh = fopen(fn, 'r');
    ...
    for ii = 1:length(pi_energy)
        ...
        for nn = 1:length(pi_energy{ii}.n)
            ...
            for mm = 1:length(pi_energy{ii}.m_data(nn).m)         
                ...
                %avgval = avgval + mean(sum3coeff(:))/normScale^2;
                %sum3coeff2 = sum(tmpRes.^2,3);
                %sq_avgval = sq_avgval + mean(sum3coeff2(:))/normScale^2;                        
                %arr2add = sum3coeff./normScale^2;
    ...
    disp(['Calculated frame : ' num2str(frameVec(ff))]);
		save('wav_pi_tmn_ek02_fixed.mat', 'pi_energy', 'pi_energy2','frame_energy_pi','frame_energy_pi2','-v7.3');
