...
for kk = 1:2:length(heightVec)
    counter = counter + 1;
    plot(kx_delta_fourier, fourier_spectra_u_frame(kk,:)./ustar^2./BLH, LineStyles{counter}); 
    plot(kwave_delta, wavelet_spectra_u_frame(kk,:)./ustar^2./BLH,'o'); 
...
text(25, 0.000001, '-5/3','FontSize',10);
xx = [0.4 10];
loglog(xx,xx.^(-1).*0.000035,'LineWidth',2); % -1 slope
text(2.5, 0.000033, '-1','FontSize',10);
text(0.1,0.1,'(b)','Units','Normalized','FontSize',11);
set(gca,'XScale','log'); set(gca,'YScale','log');
SPR = sprintf('%s%s','','wavelet_n_fourier_spectra_ek10.eps');  
set(gcf, 'Color', 'w');
print(formatEng,SPR);